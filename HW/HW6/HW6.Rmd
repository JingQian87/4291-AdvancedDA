...
rates2<-rep(c(25,50,75,100,125,150),4)
...
From the ANOVA table above, we could see that the p-value for the test is 0.016572, which is smaller than $\alpha=0.05$.
So we could reject $H_0$, which means that the average yields are different for the 6 seeding rates.

```{r}
Anova(lm(yield~rates2+field))
```
...
#data<-data.frame(period,cow,trt,yield)
#data
```
